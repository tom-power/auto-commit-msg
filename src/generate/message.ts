...
import { mostCommonParent } from "../lib/commonPath";
...
/**
 * Describe changes for multiple files using the most common parent directory.
 *
 * Finds the directory shared by the most changed files and produces a short
 * description like "update house". Falls back to undefined when there is no
 * useful common parent.
 */
export function commonPathDesc(changes: FileChange[]): string | undefined {
  const EXCLUDED = ["src", "dist", "build"];
  const paths = changes.map(item => item.from);
  const common = mostCommonParent(paths, "/", EXCLUDED);

  if (!common) {
    return undefined;
  }

  const lastSegment = path.basename(common);
  ...
  if (action === ACTION.UNKNOWN || !action) {
    return `update ${lastSegment}`;
  }

  return `${action} ${lastSegment}`;
}
